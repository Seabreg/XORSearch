/*
	29/12/2006 - 26/12/2018
	XORSearch V1.11.2, search for a XOR, ROL, ROT, SHIFT or ADD encoded string in a file
...
int ParseArgs(int argc, char **argv, int *piSave, int *piMaxLength, int *piIgnoreCase, char **ppcFile, char **ppcSearch, char **ppcSearchFile, int *piUnicode, int *piNeighbourgLength, int *piHex, int *piKeys, int *piPEFile, int *piExcludeByte, int *piWildcard, int *piWildcardEmbedded, char **ppcDisable, int *piList, int *piHexFile, int *piReverse, int *piStrings)
{
	int iIterArgv;
	...
	*piStrings = 0;
	...
	if (1 == *piList)
		...
		return 1;
	else if ((*piPEFile != 0 || *piStrings != 0) && *piNeighbourgLength != -1)
		return 1;
...
int main(int argc, char **argv)
{
	...
	if (ParseArgs(argc, argv, &iFlagSave, &iMaxLength, &iFlagIgnoreCase, &pcArgFile, &pcArgSearch, &pcArgSearchFile, &iFlagUnicode, &iNeighbourgLength, &iFlagHex, &iFlagKeys, &iFlagPEFile, &iExcludeByte, &iFlagWildcard, &iFlagWildcardEmbedded, &pcDisable, &iFlagList, &iFlagHexFile, &iFlagReverse, &iFlagStrings))
	{
		fprintf(stderr, "Usage: XORSearch [-siuhkpwWLxrS] [-l length] [-n length] [-f search-file] [-e byte] [-d encodings] file [string|hex|rule]\n"
										...
										"Use -f to provide a file with search strings\n"
										"Use -n length to print the length neighbouring characters (before & after the found keyword)\n"
										"Use -h to search for hex strings\n"
										"Use -k to decode with embedded keys\n"
